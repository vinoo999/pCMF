...
## organization of V
signalBlockV <- matrix(rep(0.7, each=nblockV), nrow=nblockV, ncol=K)
...
data1 <- dataGeneration(n=n, p=p, K=K, alpha1=alpha1, alpha2=alpha2, beta1=beta1, beta2=beta2,
                        ZI=FALSE, prob1=NULL, reorder=TRUE)
str(data1)
...
matrixHeatmap(res1$sparseParams$probSparse[,myOrder])
hist(res1$sparseParams$probSparse)
hist(as.numeric(data1$beta1>1))
