...
/*!
* \file gamPoisFactorSparseZI.cpp
...
namespace countMatrixFactor {
    ...
    // CONSTRUCTOR
    gamPoisFactorSparseZI::gamPoisFactorSparseZI(int n, int p, int K, const MatrixXi &X,
    ...
    // DESTRUCTOR
    gamPoisFactorSparseZI::~gamPoisFactorSparseZI() {}
    ...
    */
    void gamPoisFactorSparseZI::Init() {
        ...
        // Bernoulli proba (zero-inflation)
        for(int j=0; j<m_P; j++) {
            for(int i=0; i<m_N; i++) {
                if(m_X(i,j) != 0) {
     ...
     */
    double gamPoisFactorSparseZI::computeELBO() {
        ...
        for(int j=0; j<m_P; j++) {
            for(int k=0; k<m_K; k++) {
                if((m_probZIprior(j)>0) && (m_probZIprior(j)<1)) {
                    res9 += m_probZI(j,k) * std::log(m_probZIprior(j)) + (1-m_probZI(j,k)) * std::log(1-m_probZIprior(j));
                }
                if((m_probZI(j,k)>0) && (m_probZI(j,k)<1)) {
                    res10 += m_probZI(j,k) * std::log(m_probZI(j,k)) + (1-m_probZI(j,k)) * std::log(1-m_probZI(j,k));
                }
            }
        }
        resFinal += res9 - res10;
    ...
    void gamPoisFactorSparseZI::poissonRate() {
        m_lambda = m_probZI.array() * (m_EU * (m_probSparse.array() * m_EV.array()).matrix().transpose()).array();
    }
    ...
    */
    void gamPoisFactorSparseZI::multinomParam() {
    ...
    */
    void gamPoisFactorSparseZI::localParam() {
    ...
    */
    void gamPoisFactorSparseZI::globalParam() {
    ...
    */
    void gamPoisFactorSparseZI::Sproba() {
    ...
    */
     ...
     */
    void gamPoisFactorSparseZI::priorSproba() {
     ...
     * \brief update rule for Bernoulli parameter (of ZI indicator) in prior
     */
    void gamPoisFactorSparseZI::priorZIproba() {
     ...
     */
    void gamPoisFactorSparseZI::localPriorParam() {
     ...
     */
    void gamPoisFactorSparseZI::globalPriorParam() {
    ...
    */
    void gamPoisFactorSparseZI::updateEstep() {
    ...
    */
    void gamPoisFactorSparseZI::updateMstep(int iter) {
        m_curIter = iter;
     ...
     */
    void gamPoisFactorSparseZI::returnObject(Rcpp::List &results) {
        ...
        Rcpp::List returnObj2 = Rcpp::List::create(Rcpp::Named("ZIparams") = ZI,
                                                   Rcpp::Named("sparseParams") = sparse);
