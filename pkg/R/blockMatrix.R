...
#'
#' @details
#' The matrix is constructed by blocks in rows and columns,
...
#' @param signalBlock matrix \code{nRowBlock} x \code{nColBlock} of parameters used to generate
...
blockMatrix = function(nrow, ncol, nRowBlock, nColBlock, signalBlock=NULL) {
    ...
    for(rowBlock in id.rowblock) {
        for(colBlock in id.colblock) {
            ...
            mat[rowsBlock, colsBlock] = matrix(signalBlock[rowBlock, colBlock], nrow=nrowsBlock, ncol=ncolsBlock) #matrix(signalBlock[rowBlock, colBlock], nrow=nrowsBlock, ncol=ncolsBlock)
        }
    }
